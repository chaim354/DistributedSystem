...
public class ZooKeeperLeaderElection {
    ...
    public synchronized Vote lookForLeader() {
        ...
        while (this.myPeerServer.getPeerState() == ZooKeeperPeerServer.ServerState.LOOKING || this.myPeerServer.getPeerState() == ZooKeeperPeerServer.ServerState.OBSERVER) {
           // logger.info(myPeerServer.getServerId() +  " is stuck in outer hell " + myPeerServer.getPeerEpoch() );
            ...
            while (message == null) {
                ...
                if(!message.getMessageType().equals(MessageType.ELECTION)){
                    message=null;
                    continue;
                }
                en = getNotificationFromMessage(message);

                if(myPeerServer.isPeerDead(en.getSenderID())|| myPeerServer.getPeerEpoch()!= en.getPeerEpoch()){
                    message=null;
                    continue;
                }
    ...
    private boolean isHigherVote() {
        ...
        try {
            while ((m = this.incomingMessages.poll(finalizeWait, TimeUnit.MILLISECONDS)) != null) {
                ...
                if(myPeerServer.isPeerDead(n.getProposedLeaderID())|| n.getPeerEpoch()!= myPeerServer.getPeerEpoch() || n.getState().equals(ServerState.OBSERVER)){
                    continue;
                }
